	...
	function requeststatusStatus() {
		request.get( {url : url, headers : { "Authorization" : auth } }, function (error, response, body) {
			if (response === null || response === undefined) {
				self.emit("error", 'Can not connect to mower. Is mower online?');
				if (TRACE) console.log('Can not connect to mower. Is mower online?');
				return;
                        }
			if (response.statusCode) self.emit("responseStatusCode", response.statusCode);
